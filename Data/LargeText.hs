...
import GHC.Exts
import GHC.ST
import Turbo.Prelude hiding (uncons, getLine)
...
chunkOfChar txt c = let gc = charOffset txt +# c
            in let !(# m, o #) = gc `divModInt#` 512#
            in let mk = marker txt m
...
finalChunk txt =
        let ms = markers txt
        in let m = marker' ms (sizeofSmallArray# ms)
...
indexPos txt (I# i) = if inBounds txt i
        then
            let !(# d, ch, m #) = chunkOfChar txt i in
            let (l, r) = T.splitAt (I# d) ch in
            let !(# ln, cn #) = computePos (line m) (col m) (T.unpack l) in
                Just (T.head r, Position (I# ln) (I# cn))
        else
            Nothing
...
posOfChar txt (I# i) = if inBounds txt i
        then let !(# d, ch, m #) = chunkOfChar txt i in
            let !(# r, l #) = computePos (line m) (col m) (take (I# d) (unpack ch)) in
            Just (Position (I# r) (I# l))
        else
            Nothing
...
charAtPos tx p = case seekMarker tx p of
        ...
        (#   i, m #) -> case seekPos tx p m of
            ...
            j   -> let k = i *# 512#  +#  j  -#  charOffset tx in
                    if inBounds tx k then Just (I# k) else Nothing
 where
        ...
        seekPos txt (Position (I# lW) (I# cW)) (Marker b l0 c0) = f l0 c0 0# (chunkFromByte txt b) where
        ...
        seekMarker txt (Position (I# ln) (I# co)) =
                let !(# lo, hi #) = markerRange txt in
                case binSeekSArr# (markers txt) before lo hi of
                    (# -1#, _ #) -> if before zeroMarker
                        then (#  0#, zeroMarker #)
                        else (# -1#, undefined #)
                    (#   i, x #) -> (# inc# i, x #)
         where
                ...
                binSeek# f = seek where
                        seek :: Int# -> Int# -> Int#
                        seek l h = if l `geq#` h
                                then if f l
                                    then l
                                    else -1#
                                else let m = l +# ((h -# l +# 1#) `divInt#` 2#) in
                                    if f m
                                        then seek m h
                                        else seek l (m -# 1#)
...
getLine txt ln = charAtPos txt (Position ln 1) <§ \i ->
...
instance ISpan LargeText where
    ...
    baseSpan txt@(LargeText bs ms _ _) =
            ...
            in let !(I# z) = T.length$ finalChunk txt
...
fromText = \t -> runST (ST (f t))
 where
        ...
        f txt@(Text (ByteArray bs) _ _) s0 = let !(# s1, arr #) = newSmallArray# 8# (undefined :: Marker) s0 in
                let !(# s2, cc, ms #) = initMarkers 8# arr 0# txt s1 in
        ...
        advanceMarker m0 t0 = adv 0# m0 t0
         where
            ...
            adv n    m t =
                    ...
                    let !(Text _ _ (I# l),t') = T.splitAt 1 t in
                    case t @ 0 of
                        Nothing   -> Left (I# n)
        ...
        initMarkers c arr n txt s | n `geq#` c = let c' = c *# 2# in
                                                  let !(# s', arr' #) = resizeSmallMutableArray# arr c' undefined s in
                                                  initMarkers c' arr' n txt s'
                                  | otherwise  = let !(# s1, m #) =  if n `eq#` 0# then (# s, zeroMarker #) else readSmallArray# arr (n -# 1#) s in
                                                    case advanceMarker m txt of
                                                        Left (I# o) -> let s2 = shrinkSmallMutableArray# arr n s1 in
                                                                        let !(# s3, arr' #) = unsafeFreezeSmallArray# arr s2 in
                                                                        (# s3, o  +#  512# *# n, arr' #)
                                                        Right (!m', !t') -> let s2 = writeSmallArray# arr n m' s1 in
                                                                            initMarkers c arr (inc# n) t' s2
